 ...
 * 
 *<p>A control string is a Java string that contains a control specification. 
 * The control specification starts at the first percent sign (%) in the string, 
 * provided that this percent sign
 *<ol><li>is not escaped protected by a matching % or is not an escape % 
 * character,
 *<li>is not at the end of the format string, and
 *<li>precedes a sequence of characters that parses as a valid control 
 * specification.</ol></p>
 *<p>A control specification usually takes the form:
 *<pre> % ['-+ #0]* [0..9]* { . [0..9]* }+
 *                { [hlL] }+ [idfgGoxXeEcs]</pre> There are variants of this 
 * basic form that are discussed below.</p>
 *<p>The format is composed of zero or more directives defined as follows:
 *<ul><li>ordinary characters, which are simply copied to the output stream;
 ...
 * format in the argument list, rather than to the next unused argument.  In 
 * this case, the conversion characer % is replaced by the sequence 
 ...
 *<table>
 *<tr><th align=left>Sequence</th><th align=left>Name</th><th 
 * align=left>Description</th></tr>
 *<tr><td>\\</td><td>backlash</td><td>None.</td></tr>
 *<tr><td>\a</td><td>alert</td><td>Attempts to alert the user through audible or 
 * visible notification.</td></tr>
 *<tr><td>\b</td><td>backspace</td><td>Moves the printing position to one column
 * before the current position, unless the current position is the start of a 
 * line.</td></tr>
 *<tr><td>\f</td><td>form-feed</td><td>Moves the printing position to the 
 * initial printing position of the next logical page.</td></tr>
 ...
 * start of the current line.</td></tr>
 *<tr><td>\t</td><td>tab</td><td>Moves the printing position to the next 
 * implementation-defined horizontal tab position.</td></tr>
 *<tr><td>\v</td><td>vertical-tab</td><td>Moves the printing position to the 
 *start of the next implementation-defined vertical tab position.</td></tr>
 *</table></p>
 *
 ...
 * the the minimum number of characters to be printed.</p>
 *<p>An optional precision that gives the minumum number of digits to appear for 
 ...
 * appears with a c conversion character the precision is ignored.</p>
 ...
 * before printing).</p>
 *<p>An optional l (ell) specifies that a following d, i, o, x, or X conversion 
 ...
 *<p>In format strings containing the %<code>n</code>$ form of a conversion 
 ...
 *      other conversions the flag is ignored.  The non-monetary grouping 
 *      character is used.
 ...
 *      (It will begin with a sign only when a negative value is converted if 
 *      this flag is not specified.)
 * <dt>&lt;space&gt;<dd> If the first character of a signed conversion is not a 
 *      sign, a space character will be placed before the result. This means 
 *      that if the space character and + flags both appear, the space flag will 
 *      be ignored.
 * <dt>#<dd> value is to be converted to an alternative form.  For c, d, i, and 
 ...
 *      or X conversion, a non-zero result has 0x or 0X prefixed to it, 
 *      respectively. For e, E, f, g, and G conversions, the result always 
 *      contains a radix character, even if no digits follow the radix character 
 *      (normally, a decimal point appears in the result of these conversions 
 *      only if a digit follows it).  For g and G conversions, trailing zeros 
 ...
 *      appear, the 0 flag is ignored.  For d, i, o, x, and X conversions, if a 
 *      precision is specified, the 0 flag will be ignored. For c conversions, 
 *      the flag is ignored.
 *</dl>
 *
 ...
 *        digits, it will be expanded with leading zeros.  The default precision 
 *        is 1.  The result of converting 0 with an explicit precision of 0 is 
 *        no characters.
 * <dt>o<dd> The int argument is converted to unsigned octal format in the style 
 *        ddddd.  The precision specifies the minimum number of digits to 
 *        appear;  if the value being converted can be represented in fewer 
 *        digits, it will be expanded with leading zeros.  The default precision 
 *        is 1.  The result of converting 0 with an explicit precision of 0 is 
 *        no characters.
 * <dt>x<dd> The int argument is converted to unsigned hexadecimal format in the 
 *        style dddd;  the letters abcdef are used.  The precision specifies the 
 *        minimum numberof digits to appear; if the value being converted can be 
 ...
 *        specification.  A Locale is used to determine the radix character to 
 *        use in this format. If the precision is omitted from the argument, 
 *        six digits are written after the radix character;  if the precision 
 *        is explicitly 0 and the # flag is not specified, no radix character 
 ...
 *        [-]d.ddde{+-}dd (the symbols {+-} indicate either a plus or minus 
 *        sign), where there is one digit before the radix character (shown here 
 *        as a decimal point) and the number of digits after it is equal to the 
 *        precision.  A Locale is used to determine the radix character to use 
 *        in this format.  When the precision is missing, six digits are written 
 *        after the radix character; if the precision is 0 and the # flag is not 
 *        specified, no radix character appears.  The E conversion will produce 
 ...
 *        written requires an exponent greater than two digits, additional 
 *        exponent digits are written as necessary.  The value is rounded to the 
 *        appropriate number of digits.
 * <dt>g,G<dd>The floating point number argument is written in style f or e (or 
 *        in sytle E in the case of a G conversion character), with the 
 *        precision specifying the number of significant digits.  If the 
 *        precision is zero, it is taken as one.  The style used depends on the 
 *        value converted:  style e (or E) will be used only if the exponent 
 *        resulting from the conversion is less than -4 or greater than or equal 
 *        to the precision. Trailing zeros are removed from the result. A radix 
 ...
 *        all characters up to the end of the string are written.
 ...
 * created.</p>
 *<p>If a floating point value is the internal representation for infinity, the 
 ...
 * field is simply expanded to contain the conversion result.</p>
 ...
 *<p>The biggest divergence from the C printf specification is in the use of 16
 ...
 * omissions are present because Java never uses bytes to represen characters 
	 ...
	 */
		...
		private String printOFormat(final short x) {
			...
			if (x == Short.MIN_VALUE) {
				...
				final String t = Integer.toString((~(-x - 1)) ^ Short.MIN_VALUE, 8);
				switch (t.length()) {
					case 1:
						sx = "10000" + t;
						break;
		...
		private String printXFormat(final long x) {
			...
			if (x == Long.MIN_VALUE) {
				...
				final String t = Long.toString((~(-x - 1)) ^ Long.MIN_VALUE, 16);
				switch (t.length()) {
					case 1:
